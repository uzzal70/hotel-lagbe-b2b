...
const CustomerFares = ({ allData, bookingId }) => {
  // this api is called
  ...
  const data = customerData?.[0]?.customerFare ?? [];

  const totalPrice = customerData ? customerData?.[0]?.totalPrice : 0;
  const result = data || [];
  const columns = [
    {
      Header: 'Pax Type',
  ...
  function calculateTotalDiscount(data) {
    ...
    data?.forEach((passenger) => {
      if (passenger?.passengerType === 'ADT') {
        totalDiscount +=
          parseFloat(passenger.discount)?.toFixed(2) *
          parseInt(allData?.adultCount);
      } else if (passenger?.passengerType === 'CNN') {
        totalDiscount +=
          parseFloat(passenger.discount)?.toFixed(2) *
          parseInt(allData?.childCount);
      } else if (passenger?.passengerType === 'INF') {
        totalDiscount +=
          parseFloat(passenger.discount)?.toFixed(2) *
          parseInt(allData?.infantCount);
      }
    });
  ...
  return (
    <Box>
      {data?.length === 0 ? (
        <FareDetails data={allData?.passengers} allData={allData} />
      ) : (
        <Box>
          <CustomTable
            columns={columns}
            ...
            <Grid item xs={12} sm={6} md={4} lg={3} px={1}>
              ...
              >
                ...
                >
                  ...
                  <Box>
                    {'—'}&nbsp;
                    {customerData ? customerData?.[0]?.discount : 0}
                  </Box>
                  <Box>{customerData ? customerData?.[0]?.ait : 0}</Box>
                </Box>
              </Stack>
              <Stack
                direction="row"
              ...
              >
                ...
                <Box>
                  ...
                  {`${(
                    Number(totalPrice) +
                    Number(customerData ? customerData?.[0]?.ait : 0)
                  ).toFixed(2)}`}
                  {/* BDT {totalPrice + parseFloat(allData?.ait || 0)?.toFixed(2)} */}
                </Box>
              </Stack>
            </Grid>
          </Grid>
