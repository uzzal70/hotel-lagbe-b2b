...
const InstantDeposit = () => {
  ...
  const bankData = [
    ...
    // {
    //   id: 'f1fef289-b9eb-4d6a-9364-e49395b5159b',
    //   bankingName: 'USD Deposit',
    //   logo: ImageImport.stripe,
    //   offer: '(1% Cashback offer)',
    //   charge: '0% Payment Charge',
    //   provider: 'SSL Commerz',
    //   accountNo: '1212121',
    //   accountName: 'SSL Commerz',
    //   transactionCharge: '122',
    //   instantAvailability: true,
    //   status: true,
    //   createdAt: '2024-08-08T06:43:20.488Z',
    //   updatedAt: '2024-08-08T06:43:20.488Z',
    //   link: 'https://pay.airwallex.com/sgh67hiu1ibl',
    // },
  ];
  const dollarRate = 122;
  const dollarpaymentLink = 'https://pay.airwallex.com/sgh67hiu1ibl';
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    const urlSSL = `${baseUrl}/core/deposit-request/instantBankPayment?agentId=${agentId}&totalAmount=${percentage(
      formik?.values?.service,
      formik?.values?.amount || 0
    )}&depositAmount=${formik.values.amount}&platform=${companyInfo.platform}`;

    const mtbUrl = `${baseUrl}/core/deposit-request/intantMTBPayment?agentId=${agentId}&totalAmount=${percentage(
      formik?.values?.service,
      formik?.values?.amount || 0
    )}&depositAmount=${formik.values.amount}&phone=${
      tokenise?.phone
    }&platform=${companyInfo.platform}`;
    try {
      ...
      if (responseData?.status === 'success') {
        ...
        Swal.fire({
          ...
          text: responseData?.message || 'Failed to make the payment',
        });
      }
    } catch (error) {
      Swal.fire({
        ...
        <Container sx={{ px: { xs: 1, sm: 2, md: '' } }}>
          ...
          >
                ...
                {bankData?.map((button, i, arr) => (
                  ...
                  >
                    <Button
                      onClick={() =>
                        //    {
                        //   if (button.bankingName === 'USD Deposit') {
                        //     window.open(button.link, '_blank'); // Open the link in a new tab
                        //   } else {
                        //     handleChange(button.bankingName);
                        //   }
                        // }
                        handleChange(button.bankingName)
                      }
                    ...
                    >
                      ...
                      <Box color={button.offer ? 'var(--primary)' : ''}>
                        {button.bankingName}
                      </Box>
                      <Box
                        sx={{
            ...
            <Box sx={{ mt: 5, mb: 2 }}>
              ...
              {formik.values.paymentType === 'USD Deposit' && (
                <Box
                  sx={{
                    bgcolor: 'var(--stroke)',
                    width: 'fit-content',
              ...
              formik.values.paymentType === 'Visa/Master' ? (
                    ...
                    {formik.values.paymentType === 'USD Deposit' && (
                      <Grid item xs={12} sm={3} md={3} lg={3}>
                        <CustomInput
                          ...
                          placeholder="Enter Amount"
                          type="number"
                          value={(formik.values.amount / dollarRate).toFixed(2)}
                          padding="10px 15px"
                          fontSize="16px"
                          border="1px solid var(--bgcolor)"
                          disabled={true}
                    ...
                    {formik.values.paymentType === 'USD Deposit' && (
                      <Grid item xs={12} sm={3} md={3} lg={3}>
                        <CustomInput
                          ...
                          name="depositAmount"
                          type="number"
                          value={percentage(1, formik.values.amount || 0)}
                          padding="10px 15px"
                          fontSize="16px"
                          border="1px solid var(--bgcolor)"
                          disabled={true}
                        ...
                        <>
                          {formik.values.paymentType === 'USD Deposit' ? (
                            <CustomButton
                              ...
                              padding={'8px 20px'}
                              handleClick={() =>
                                window.open(dollarpaymentLink, '_blank')
                              }
                            />
                          ) : (
                            <CustomButton
                ...
                >
                  Coming soon! Please make a deposit using our other methods.{' '}
                </Box>
              ) : null}
            </Box>
          </Box>
