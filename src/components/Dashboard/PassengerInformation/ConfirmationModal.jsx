...
const ConfirmationModal = ({
  ...
  const adultValue = passenger.adult;
  const childValue = passenger.child;
  const infantValue = passenger.infant;
      ...
      <Modal open={isModal} onClose={handleCloseModal}>
        ...
        >
          ...
          >
            ...
            >
              <Stack direction={'row'} justifyContent={'space-between'}>
                <Box>Review Flight Details</Box>
                <Box
                  sx={{
                    pr: { xs: 0, md: 2 },
                  }}
                >
                  Total Amount: {data?.totalClientPrice || '0'} {companyInfo.currencyType}
                </Box>
              </Stack>
            </Box>
            <Box
            ...
            >
              ...
              >
                <Box onClick={() => handleClick('flight')}>
                  <Stack
                    direction={'row'}
                    justifyContent={'space-between'}
                    alignItems={'center'}
                  >
                    <Box>Flight Details</Box>
                    <Box>
                ...
                <Collapse in={expand?.passenger}>
                  <Box className="preview-pax-table" mb={1.5}>
                    <table>
                      ...
                      <tbody>
                        {personDetails.map((pax, i) => (
                          <tr key={i}>
                            <td data-label="Pax Name">
                              {pax?.firstName} {pax?.lastName}
                            </td>
                            <td data-label="Gender">
                              {pax?.gender === 'M' || pax?.gender === 'MI'
                                ? 'Male'
                                : 'Female'}
                            </td>
                            ...
                            {locationType === 'INSIDE' ? null : (
                              <td data-label="Passport">{pax?.passportNo}</td>
                            )}
                            {locationType === 'INSIDE' ? null : (
                              <td data-label="Passport Expiry">
                                {pax?.passportEx}
                              </td>
                            )}
                          </tr>
          ...
          >
            ...
            >
              {checkButton ? 'Issue Now' : 'Book & Hold'}
            </Button>
          </Box>
        </Box>
      </Modal>
